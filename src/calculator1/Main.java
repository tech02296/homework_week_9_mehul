...
public class  Main extends Calculator { // parent class extention
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in); // scanner applied
        char choice = 'C'; // variable char 'C' for user input
        while (choice == 'y') { // while loop using
            System.out.println("Enter first number: ");//for user enter numbers
            int a = scanner.nextInt();// define value for integer from scanner
            System.out.println("Enter second number: ");
            int b = scanner.nextInt();//define value for integer from scanner
            System.out.println("Enter any symbol from +,-,/,* ");
            char c = scanner.next().charAt(0);//define value for integer from scanner
            Main obj = new Main();
            obj.calculationResult(a, b, c);//calling method
            System.out.println("Would you like to more calculation enter 'Y' for yes or 'N' for no: ");
        }
